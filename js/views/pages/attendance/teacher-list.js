...
window.loadStudentList = async () => {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/list?class_id=${classId}&date=${date}`
    );
    const result = await res.json();
    ...
    result.data.forEach((s, index) => {
      // Default 'hadir' (sesuai ENUM database Anda yang lowercase)
      // Jika s.status kosong (belum absen), defaultkan ke 'hadir'
                        ...
                        <div class="attendance-options">
                            <label class="attendance-label">
                                <input type="radio" name="status_${
                                  s.student_id
                                }" value="hadir" ${check("hadir")}> H
                            </label>
                            <label class="attendance-label">
                                <input type="radio" name="status_${
                                  s.student_id
                                }" value="izin" ${check("izin")}> I
                            </label>
                            <label class="attendance-label">
                                <input type="radio" name="status_${
                                  s.student_id
                                }" value="sakit" ${check("sakit")}> S
                            </label>
                            <label class="attendance-label">
                                <input type="radio" name="status_${
                                  s.student_id
                                }" value="alfa" ${check("alfa")}> A
                            </label>
                        </div>
                    </td>
                    
                    <td>
                        <input type="text" class="form-control note-input" value="${note}" placeholder="Ket.">
                    </td>
                </tr>
            `;
      tbody.innerHTML += row;
  ...
  try {
    ...
    if (result.success) {
      if (typeof Notifications !== "undefined")
        Notifications.success("Absensi berhasil disimpan!");
