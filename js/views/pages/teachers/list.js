...
window.handleSaveTeacher = async (e) => {
    ...
    if(!isEdit && !data.password) {
        if(typeof Notifications !== 'undefined') Notifications.error("Password wajib diisi untuk guru baru!");
    ...
    try {
        ...
        if(result.success) {
            if(typeof Notifications !== 'undefined') Notifications.success(result.message);
            else alert(result.message);
            
            window.closeModal();
            fetchTeachers();
        } else {
            if(typeof Notifications !== 'undefined') Notifications.error(result.message);
            else alert(result.message);
        }
    } catch (error) {
        if(typeof Notifications !== 'undefined') Notifications.error("Terjadi kesalahan sistem.");
...
window.deleteTeacher = async (id) => {
    ...
    Notifications.confirm("Hapus Guru?", "Akses login guru ini akan hilang.", async () => {
        deleteExecute(id);
    });
};
